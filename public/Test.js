...
Elm.Main.make = function (_elm) {
   ...
   var render = function (model) {
      return function () {
         var content = function (rot) {
            return _L.fromArray([A2($Graphics$Collage.rotate,
            $Basics.degrees($Basics.toFloat(rot)),
            $Graphics$Collage.toForm($Graphics$Element.show(model.text)))]);
         };
   ...
   m) {
      return function () {
         ...
         {case "_Tuple2":
            return function () {
                 var newText = _U.cmp(m.x,
                 200) > 0 ? $String.reverse(stateName) : stateName;
                 return _U.replace([["x",_v0._0]
                                   ,["text",newText]],
                 m);
              }();}
         _U.badCase($moduleName,
